...
CRITICAL = {
    "baby_monitor/config.yaml",
    "baby_monitor/run.sh",
    "baby_monitor/Dockerfile",
    ...
    try:
        req = Request(
            ...
            headers={
                ...
                "User-Agent": "hermes-baby-monitor-monitor",
            },
        )
...
def main() -> int:
    ...
    local_paths = set(filter(None, git("diff", "--name-only", "upstream/main..HEAD").splitlines()))
    ...
    local_patch = bool(
        ...
        & {
            ...
            "baby_monitor/run.sh",
            "baby_monitor/Dockerfile",
            "baby_monitor/apparmor.txt",
        }
    )
    tags = sorted(git("ls-remote", "--tags", "--refs", "upstream").splitlines())
    tag_names = [line.rsplit("refs/tags/", 1)[-1] for line in tags]
    releases = (
        api_json("https://api.github.com/repos/victoriano/home-assistant-baby-monitor/releases?per_page=20") or []
    )
    ...
    interesting_tags = sorted(
        {
            t
            for t in tag_names + release_names + image_tags
            if t in {"0.4.0", "latest"} or t.startswith("0.4.") or t.startswith("0.5.")
        }
    )
